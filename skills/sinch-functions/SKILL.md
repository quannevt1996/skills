...
For terminal commands (`sinch ...`) refer to the `sinch-cli` skill. For runtime code, refer to `sinch-functions-node` or `sinch-functions-dotnet`. For the Voice API v2 REST contract behind `context.voice` refer to the Voice API 2.0 documentation at https://developers.sinch.com/docs/voice-2.0; no skill covers Voice 2.0 yet.
