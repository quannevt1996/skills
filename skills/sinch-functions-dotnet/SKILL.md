...
description: Write C#/.NET Sinch Functions code (NuGet package `Sinch.Functions.Runtime`, target `.NET 10`). Use when writing or editing a `FunctionController`, extending `SinchVoiceController`/`SinchConversationController`/`SinchController`/`ElevenLabsController`, overriding `Handlers` with a `CallHandlers` object (`Incoming`/`Answered`/`Manage`/`Completed`), building call flows with `CommandBuilder` (`Answer`/`Say`/`Dial`/`Menu`/`Hangup`/`Build`), handling `call.incoming`/`call.answered`/`call.menu`/`call.hangup` events, placing calls with `Context.Voice`, registering DI via `ISinchFunctionInit.ConfigureServices`, using `FunctionContext` (cache/storage/database/SDK clients like `Context.Sms`/`Context.Voice`/`Context.Conversation`), protecting actions with `[Authorize]`, or using helpers like `VoiceErrorHelper` and `ConversationMessage`. Also covers the legacy Voice v1 `Ice`/`Ace`/`Pie`/`Dice` overrides with `IceCallbackModel`/`PieCallbackModel` and the `IceSvamletBuilder`/`AceSvamletBuilder`/`PieSvamletBuilder` `Instructions.* → Action.* → Build()` chain. For terminal commands (`sinch functions dev`, `sinch functions deploy`) use sinch-cli; for Node.js runtime code use sinch-functions-node.
metadata:
  author: Sinch
  version: 1.0.0
  category: Functions
